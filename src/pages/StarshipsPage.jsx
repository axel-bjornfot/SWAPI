...
const StarshipsPage = () => {
	...
	const { data, error, isError, isFetching, isLoading, isPreviousData } =
		useQuery(["starships", page], () => getStarships(page), {
	...
	return (
		<Container className="py-3">
			...
			<div>
				{isLoading && <p className="my-3">Loading starships...</p>}

				{isError && (
					<p className="my-3">
						Sorry, tried to get starships but no stamps found (
						{error})
					</p>
				)}
				...
				{data?.results.results && (
					<>
						{data.results.results.map((starship, i) => (
							<Card key={i}>
