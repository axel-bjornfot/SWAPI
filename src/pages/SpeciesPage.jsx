...
const SpeciesPage = () => {
	...
	const { data, error, isError, isFetching, isLoading, isPreviousData } =
		useQuery(["species", page], () => getSpecies(page), {
	...
	return (
		<Container className="py-3">
			...
			<div>
				{isLoading && <p className="my-3">Loading species...</p>}

				{isError && (
					<p className="my-3">
						Sorry, tried to get species but no stamps found ({error}
						)
					</p>
				)}
				...
				{data?.results.results && (
					<>
						{data.results.results.map((species, i) => (
							<Card key={i}>
